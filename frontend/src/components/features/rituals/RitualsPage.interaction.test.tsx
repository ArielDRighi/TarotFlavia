...
import { RitualsPage as RitualesPage } from './RitualsPage';
...
/** Resultado de query parcial, tipado como lo espera `vi.mocked(hook)`. */
function queryResult<T>(partial: { data?: T; isLoading: boolean }): UseQueryResult<T, Error> {
...
/**
 * Tests de interacción de `RitualsPage` (filtros, búsqueda, destacados).
 * Vivían en `app/rituales/page.test.tsx`; desde T-SEO-015 la ruta es un Server
 * Component que siembra los rituales, así que el componente se prueba solo.
 */
describe('RitualsPage (interacción)', () => {
  beforeEach(() => {
    vi.clearAllMocks();
    vi.mocked(useAuthStore).mockReturnValue({
      isAuthenticated: false,
    });
    vi.mocked(useRitualCategories).mockReturnValue(
      queryResult({
        data: [
    ...
    vi.mocked(useFeaturedRituals).mockReturnValue(
      queryResult({
        data: [mockRituals[0]],
        isLoading: false,
      })
  ...
  it('⚠️ T-SEO-015: siembra el listado con los rituales resueltos en el servidor (sin filtros)', () => {
  ...
  it('filters by category', async () => {
    ...
    vi.mocked(useRituals).mockImplementation(mockUseRituals);

    render(<RitualesPage />);

    // Initially called with empty filters (and the SSR seed as second argument, T-SEO-015)
